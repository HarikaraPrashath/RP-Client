        ...
        <header className={styles.hero}>
          <div className={styles.heroText}>
            ...
            <p className={styles.lead}>
              A colorful snapshot of how every role aligns with your profile: strengths, gaps, and
              the skills showing up most often.
            </p>
              ...
              {topMissingSkills.length === 0 ? (
                ...
                topMissingSkills.map((item) => (
                  <span key={item.skill} className={styles.skillChipSoft}>
                    {item.skill} <span className={styles.skillCount}>×{item.count}</span>
                  </span>
                ))
              )}
            </div>
            ...
            {bestMatch ? (
              <>
                <p className={styles.highlightRole}>
                  ...
                  <span>Match: {bestMatch.match_percent}%</span>
                </p>
            ...
            {lowestMatch ? (
              <>
                <p className={styles.highlightRole}>
                  ...
                  <span>Match: {lowestMatch.match_percent ?? 0}%</span>
                </p>
            ...
            ranked.map((job, idx) => {
              ...
              return (
                <article key={`${job.ref ?? "job"}-${idx}`} className={styles.card}>
                  ...
                  {overlap.length > 0 ? (
                    <div className={styles.skillsRow}>
                      <p className={styles.subLabel}>Matched</p>
                      <div className={styles.chips}>
                        {overlap.map((skill) => (
                          <span key={skill} className={styles.matchChip}>
                            {skill}
                  ...
                  {missing.length > 0 ? (
                    <div className={styles.skillsRow}>
                      <p className={styles.subLabel}>Missing</p>
                      <div className={styles.chips}>
                        {missing.map((skill) => (
                          <span key={skill} className={styles.missingChip}>
                            {skill}
