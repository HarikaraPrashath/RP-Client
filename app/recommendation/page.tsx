...
import { Target, Briefcase, Heart, TrendingUp } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const renderLoadingState = () => (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-b from-background via-primary/5 to-background">
      <div className="text-center">
        <div className="w-16 h-16 border-4 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
        <p className="text-foreground/70 font-medium">Loading profile...</p>
      </div>
    </div>
  );
  ...
  const renderEmptyState = () => (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-b from-background via-primary/5 to-background">
      <div className="text-center">
        <p className="text-foreground/70 text-lg mb-4">No user found</p>
        <Link
          href="/login"
    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-primary/5 to-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <TabNavigation
          ...
          onGenerateReport={() => generateRecommendationReport(user, profileData)}
        />
        ...
        <div className="mt-8 flex justify-center">
          <Link
            href="/"
            className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-primary to-accent 
                     text-white font-medium rounded-lg transition duration-200 
                     focus:ring-4 focus:ring-primary/50 shadow-lg hover:shadow-xl
...
// Profile banner removed as requested
...
function TabNavigation({ tabs, activeTab, onTabChange, onGenerateReport }: TabNavigationProps) {
  return (
    <nav className="border-b border-gray-200 flex justify-between items-center" aria-label="Profile tabs">
      <div className="flex space-x-8 overflow-x-auto pb-px">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => onTabChange(tab.id)}
            className={`py-4 px-1 text-sm font-medium border-b-2 transition whitespace-nowrap flex items-center gap-2
              ${activeTab === tab.id
                ? "border-primary text-primary"
                : "border-transparent text-foreground/50 hover:text-foreground/80 hover:border-gray-300"
              }`}
            aria-current={activeTab === tab.id ? "page" : undefined}
          >
            {tab.id === 'career' && <Target className="w-4 h-4" />}
            {tab.id === 'prep' && <Briefcase className="w-4 h-4" />}
            {tab.id === 'emotional' && <Heart className="w-4 h-4" />}
            {tab.id === 'market' && <TrendingUp className="w-4 h-4" />}
            {tab.label}
          </button>
        ))}
      </div>
      ...
      <button
        ...
        className="hidden sm:inline-flex items-center gap-2 px-4 py-2 mb-2 bg-gradient-to-r from-primary to-accent text-white rounded-lg 
                 font-medium transition hover:shadow-lg focus:ring-2 focus:ring-primary focus:ring-offset-2 text-sm"
      >
...
function CareerGuideContent({ data }: { data: any }) {
  if (!data || Object.keys(data).length === 0) return <p className="text-foreground/50">No Career Guide insights saved yet.</p>;
  return (
    <section className="text-foreground/80 bg-white rounded-2xl p-6 border border-primary/10 hover:shadow-xl transition-all duration-300">
      <h2 className="text-2xl font-semibold mb-4 text-primary">Career Guide Recommendation</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-gradient-to-b from-background via-primary/5 to-background rounded-lg">
          <p className="font-bold text-foreground/50 uppercase text-xs">Top Match</p>
          <p className="text-xl font-bold text-foreground font-bold">{data.top_1_prediction}</p>
        </div>
        <div className="p-4 bg-gradient-to-b from-background via-primary/5 to-background rounded-lg">
          <p className="font-bold text-foreground/50 uppercase text-xs">Other Matches</p>
          <p className="text-foreground/80">{data.top_3_predictions?.join(", ")}</p>
        </div>
      </div>
      {data.guidance && (
        <div className="mt-4 p-4 border-l-4 border-primary bg-blue-50 rounded-r-lg">
...
function CareerPredictionContent({ data }: { data: any }) {
  if (!data || Object.keys(data).length === 0) return <p className="text-foreground/50">No Preparation Roadmap saved yet.</p>;
  return (
    <section className="text-foreground/80 bg-white rounded-2xl p-6 border border-primary/10 hover:shadow-xl transition-all duration-300">
      <h2 className="text-2xl font-semibold mb-4 text-accent">Personalized Learning Roadmap</h2>
      {data.roadmap?.milestones?.map((m: any, idx: number) => (
        <div key={idx} className="mb-4 flex items-start gap-4">
          <div className="w-8 h-8 rounded-full bg-indigo-100 text-primary flex items-center justify-center shrink-0 font-bold">{idx + 1}</div>
          <div>
            ...
            <div className="mt-2 flex gap-2 flex-wrap">
              {m.skills?.map((skill: string) => (
                <span key={skill} className="px-2 py-1 bg-gray-100 text-foreground/70 rounded-md text-xs">{skill}</span>
              ))}
            </div>
          </div>
        </div>
...
function EmotionalIntelContent({ data }: { data: any }) {
  if (!data || Object.keys(data).length === 0) return <p className="text-foreground/50">No Emotional Insights saved yet.</p>;
  const topCareer = data.topCareers?.[0];
  return (
    <section className="text-foreground/80 bg-white rounded-2xl p-6 border border-primary/10 hover:shadow-xl transition-all duration-300">
      <h2 className="text-2xl font-semibold mb-4 text-secondary">Emotional Intelligence Results</h2>
      <p className="text-lg font-medium">Emotional Match: <span className="font-bold text-secondary">{topCareer?.career}</span> ({(topCareer?.confidence || 0) * 100}%)</p>

      <div className="mt-4">
        <h3 className="font-bold mb-2">Qualitative Insights</h3>
        <ul className="list-disc pl-5 space-y-1">
          {data.insights?.map((ins: string, idx: number) => (
            <li key={idx} className="text-foreground/70">{ins}</li>
          ))}
        </ul>
      </div>
    </section>
...
function MarketTrendsContent({ data, fullProfile }: { data: any, fullProfile?: any }) {
  if (!data || (!data.allTrend && !data.mergeSkills)) {
    return (
      <div className="bg-white rounded-2xl p-8 border border-primary/10 hover:shadow-xl transition-all duration-300 text-center">
        <p className="text-foreground/50 mb-4">No Market or Skill Radar insights saved yet.</p>
        <div className="flex justify-center gap-4">
          ...
          <Link href="/career-market/merge-skills" className="text-primary hover:underline text-sm font-medium">Check Skill Radar</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-gradient-to-br from-primary to-accent text-white p-6 rounded-xl shadow-md">
        <h2 className="text-xl font-bold mb-3 flex items-center gap-2">
          <svg className="w-6 h-6 text-white/80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
        ...
        <p className="text-white/90 text-lg leading-relaxed">
          ...
          This is an excellent position, as the market for {targetRole} is currently experiencing a <strong className="text-white bg-white/20 px-1 rounded">+{growth}% surge</strong>.
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="bg-white rounded-2xl p-6 border border-accent/20 hover:shadow-xl transition-all duration-300 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1 h-full bg-accent"></div>
          <h3 className="text-lg font-bold mb-3 text-foreground font-bold flex items-center gap-2">
            <span className="text-xl">⚠️</span> High Value Gap
          </h3>
          <p className="text-foreground/80">
            You are missing <strong className="text-accent">{urgentSkill}</strong>.
          ...
          <div className="mt-4">
            <Link href="/career-market/merge-skills" className="inline-block px-4 py-2 bg-accent/10 hover:bg-accent/20 text-accent text-sm font-bold rounded-lg transition">
        ...
        <section className="bg-white rounded-2xl p-6 border border-secondary/20 hover:shadow-xl transition-all duration-300 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1 h-full bg-secondary"></div>
          <h3 className="text-lg font-bold mb-3 text-foreground font-bold flex items-center gap-2">
            <span className="text-xl">🔥</span> Your Market Edge
          </h3>
          <p className="text-foreground/80">
            You already possess solid experience in <strong className="text-secondary">{advantageSkill}</strong>,
      ...
      <section className="bg-white rounded-2xl p-6 border border-primary/10 hover:shadow-xl transition-all duration-300">
        ...
        <div className="relative pt-8 pb-4 px-4">
          ...
          <div className="absolute left-0 top-1/2 w-2/3 h-1 bg-primary -translate-y-1/2 rounded-full"></div>

          <div className="relative flex justify-between items-center z-10">
            <div className="flex flex-col items-center">
              <div className="w-4 h-4 bg-primary rounded-full ring-4 ring-white"></div>
              <span className="mt-2 text-xs font-bold text-foreground/50 uppercase">Now</span>
            </div>
            <div className="flex flex-col items-center">
              <div className="w-4 h-4 bg-primary rounded-full ring-4 ring-white"></div>
              <span className="mt-2 text-xs font-bold text-primary uppercase">3 Months</span>
            </div>
            <div className="flex flex-col items-center">
        ...
        <p className="text-center text-foreground/70 mt-4 max-w-2xl mx-auto">
          Based on <strong className="text-foreground/80">{experienceLevel}</strong> and the high momentum of <strong className="capitalize">{targetRole}</strong>,
